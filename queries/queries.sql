...
SELECT id, nombre, apellido1, apellido2, fecha_nacimiento FROM persona WHERE tipo = 'alumno' AND fecha_nacimiento LIKE '%1999%';
...
SELECT DISTINCT departamento.nombre AS nombre FROM departamento 
...
JOIN alumno_se_matricula_asignatura asma ON asma.id_alumno = persona.id
JOIN curso_escolar ce ON asma.id_curso_escolar = ce.id
WHERE persona.tipo = 'alumno' AND ce.anyo_inicio = 2018 AND ce.anyo_fin = 2019;
...
SELECT COUNT(id) AS total FROM persona WHERE tipo = 'alumno' AND fecha_nacimiento LIKE '%1999%';
...
SELECT departamento.nombre AS departamento, COUNT(profesor.id_profesor) AS total FROM departamento
...
SELECT grado.nombre AS grau, count(asignatura.id) AS total FROM grado
...
SELECT grado.nombre as grau, asignatura.tipo as tipo, SUM(asignatura.creditos) as total_creditos
FROM grado 
...
SELECT persona.id, persona.nombre, persona.apellido1, persona.apellido2, COUNT(asignatura.id) as total
FROM asignatura 
